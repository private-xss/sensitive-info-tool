<template>
  <div class="feishu-page">
    <a-card title="飞书信息管理" :bordered="false">
      <template #extra>
    ...
    >
      ...
      >
        <a-form-item label="名称" name="name">
          <a-input v-model:value="formData.name" placeholder="请输入飞书应用名称" />
        </a-form-item>
        <a-form-item label="App ID" name="appId">
          <a-input v-model:value="formData.appId" placeholder="请输入App ID" />
        </a-form-item>
...
const dataSource = ref<Feishu[]>([])

const formData = ref<Partial<Feishu>>({
  name: '',
  appId: '',
  appSecret: '',
  description: ''
...
const rules = {
  name: [{ required: true, message: '请输入飞书应用名称' }],
...
const loadData = async () => {
  loading.value = true
  try {
    const data = await SimpleStorage.loadData('sensitive_feishu') || []
    dataSource.value = data
    console.log('飞书数据加载:', data)
  } catch (error) {
...
const saveData = async (data: Feishu[]) => {
  try {
    await SimpleStorage.saveData('sensitive_feishu', data)
    console.log('飞书数据保存成功:', data)
  } catch (error) {
...
const deleteItem = async (record: Feishu) => {
  try {
...
const copyToClipboard = async (record: Feishu) => {
  try {
